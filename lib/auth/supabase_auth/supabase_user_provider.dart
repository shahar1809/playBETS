...
class BetsSupabaseUser extends BaseAuthUser {
  BetsSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> betsSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = BetsSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
